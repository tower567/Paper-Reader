...
def existing_read_state(path: Path) -> bool:
    if not path.is_file():
        return False
    ...
    try:
        ...
        return False
    return frontmatter.get("read") is True
...
def paper_page(
    ...
    frontmatter = paper_frontmatter(
        record,
        metadata,
        tracks,
        existing_read_state(repo / relative / "paper.md"),
    )
    artifacts = metadata.get("artifacts") or {}
    ...
    return "\n".join(
        [
            ...
            yaml_text,
            "---",
            "<!-- Generated by build_obsidian.py. The read checkbox is preserved; edit other fields at their source. -->",
            "",
            f"# {title}",
            "",
            meta,
...
def base_config(name: str, tag: str | None = None) -> dict[str, Any]:
    ...
    return {
        "filters": {"and": filters},
        "formulas": {
            "paper_link": 'if(read, file.asLink("✅ " + display_title), file.asLink(display_title))'
        },
        "properties": {
            "formula.paper_link": {"displayName": "论文"},
            "note.read": {"displayName": "已读"},
